...
public class Main {
	...
	public static void main(String[] args) {
		...
		try {
			...
			System.err.println("ERROR LÉXICO     (línea 1): No se reconoce el símbolo en la gramática.\n");
			errorAFichero("ERROR LÉXICO     (línea 1): No se reconoce el símbolo en la gramática.\n");
			try {
				error.close();
	...
	public static void compToken(String token){
		if(tokenActualIgual(token)){
			try{
				...
				errorAFichero("ERROR LÉXICO     (línea "+linea()+"): No se reconoce el símbolo en la gramática, después del símbolo \""+tokenActual.token+"\"\n");
				throw new RuntimeException("ERROR LÉXICO     (línea "+linea()+"): No se reconoce el símbolo en la gramática, después del símbolo \""+tokenActual.token+"\".\n");
			}
	...
	public static Entrada T() {//[19..21]	T -> int | boolean | char.
		if(tokenActualIgual("int")){//First(int) = int
			...
			parser.add(21);
			compToken("char");
			return new Entrada (0,"null","char",4,null,"null",0);
