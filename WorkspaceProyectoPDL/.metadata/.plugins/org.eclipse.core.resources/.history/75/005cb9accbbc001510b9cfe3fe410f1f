...
public class Main {
	...
	public static void main(String[] args) {
		...
		try {
			...
			System.err.println("Sintaxis incorrecta en línea("+linea()+"). Carácter \""+tokenActual.token+"\" no esperado.\n\n");//luego se quita
		}
	...
	public static void Pp(){//[1] 			P' -> P
		...
		if(tokenActualIgual("eol")
			...
			throw new RuntimeException("ERROR SINTÁCTICO  (línea "+linea()+"): No se ha aceptado ningún token en P'.");
	}
	...
	public static void Z(){//[6..7]			Z -> eol Z | .
		if(tokenActualIgual("eol")){//First(eolZ) = eol
			...
			throw new RuntimeException("ERROR SINTÁCTICO  (línea "+linea()+"): No se ha aceptado ningún token en Z.");
	}
	...
	public static Entrada B(){//[8..11]		B -> var T id | if ( E ) S | S | for ( I ; E ; A ) eol Z { eol Z C }
		if(tokenActualIgual("var")){//First(varTid) = var
			...
			throw new RuntimeException("ERROR SINTÁCTICO  (línea "+linea()+"): No se ha aceptado ningún token en B.");
	}
	...
	public static Entrada S(){//[12..15] 	S -> id S' | return X | write ( E ) | prompt ( id )
		if(tokenActualIgual("Id")){//First(idS') = id
			...
			throw new RuntimeException("ERROR SINTÁCTICO  (línea "+linea()+"): No se ha aceptado ningún token en S.");
	}
	...
	public static Entrada T() {//[19..21]	T -> int | boolean | char.
		if(tokenActualIgual("int")){//First(int) = int
			...
			throw new RuntimeException("ERROR SINTÁCTICO  (línea "+linea()+"): No se ha aceptado ningún token en T.");
		}
	...
	public static Entrada X(){//[24..25]	X -> E | .
		if(tokenActualIgual("Id") 
			...
			throw new RuntimeException("ERROR SINTÁCTICO  (línea "+linea()+"): No se ha aceptado ningún token en X.");
	}
	...
	public static Entrada L(){//[26..27]	L -> E L' | .
		if(tokenActualIgual("Id")
			...
			throw new RuntimeException("ERROR SINTÁCTICO  (línea "+linea()+"): No se ha aceptado ningún token en L.");
	}
	...
	public static Entrada F(){//[30]		F -> function T' id ( G ) eol Z { eol Z C }
		if(tokenActualIgual("function")){//First(functionT'id(G)eolZ{eolZC}) = function
			...
			throw new RuntimeException("ERROR SINTÁCTICO  (línea "+linea()+"): No se ha aceptado ningún token en F.");
	}
	...
	public static Entrada C(){//[31..32]	C -> B eol Z C | .
		if(tokenActualIgual("var") 
			...
			throw new RuntimeException("ERROR SINTÁCTICO  (línea "+linea()+"): No se ha aceptado ningún token en C.");
	}
	...
	public static Entrada G(){//[33..34]	G -> T id G' | .
		if(tokenActualIgual("int") 
			...
			throw new RuntimeException("ERROR SINTÁCTICO  (línea "+linea()+"): No se ha aceptado ningún token en G.");
	}
	...
	public static Entrada Gp(){//[35..36]	G' -> , T id G' | .
		if(tokenActualIgual(",")){//First(,TidG') = ,
			...
			throw new RuntimeException("ERROR SINTÁCTICO  (línea "+linea()+"): No se ha aceptado ningun token en G'.");
	}
	...
	public static Entrada I(){//[37..38]	I -> id = E | .
		if(tokenActualIgual("Id")){//First(id=E) = id
			...
			throw new RuntimeException("ERROR SINTÁCTICO  (línea "+linea()+"): No se ha aceptado ningún token en I.");
	}
	...
	public static Entrada A(){//[39..40]	A -> id A' | .
		if(tokenActualIgual("Id")){//First(idA') = id
			...
			throw new RuntimeException("ERROR SINTÁCTICO  (línea "+linea()+"): No se ha aceptado ningún token en A.");
	}
	...
	public static Entrada E(){//[43]		E -> R E'.
		if(tokenActualIgual("Id")
			...
			throw new RuntimeException("ERROR SINTÁCTICO  (línea "+linea()+"): No se ha aceptado ningún token en E.");
	}
	...
	public static Entrada R(){//[46]		R -> U R'.
		if(tokenActualIgual("Id")||tokenActualIgual("(")
			...
			throw new RuntimeException("ERROR SINTÁCTICO  (línea "+linea()+"): No se ha aceptado ningún token en R.");
	}
	...
	public static Entrada U(){//[49]		U -> V U'.
		if(tokenActualIgual("Id")||tokenActualIgual("(")
			...
			throw new RuntimeException("ERROR SINTÁCTICO  (línea "+linea()+"): No se ha aceptado ningún token en U.");
	}
	...
	public static Entrada V(){//[52..57]	V -> id V' | ( E ) | entero | cadena | true | false.
		if(tokenActualIgual("Id")){//First(idV') = id
			...
			throw new RuntimeException("ERROR SINTÁCTICO  (línea "+linea()+"): No se ha aceptado ningún token en V.");
	}
